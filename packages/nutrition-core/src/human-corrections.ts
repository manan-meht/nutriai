...
import type { HumanCorrectionFields } from "./types";

// Only review statuses that actually represent a judged classification (not
// "unclear photo"/"not food"/"duplicate", which say nothing about whether
// the food fields are right) are used to override the dashboard.
 ...
 * keyed by meal_logs.id, for use by the caregiver/self/coach dashboards and
 * the mobile API. Takes an already-constructed service-role client — this
 * package never constructs its own Supabase clients. */
export async function fetchHumanCorrectionsByMealLogId(
  admin: SupabaseClient,
  mealLogIds: string[]
): Promise<Record<string, HumanCorrectionFields>> {
