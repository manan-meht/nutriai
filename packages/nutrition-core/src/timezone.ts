...
//
// Shared here rather than living in the web app because every client that
// creates a contact needs the same guess: for a year the mobile app sent no
// timezone at all, the column defaulted to Asia/Kolkata, and every Singapore
// user's lunch was logged as breakfast (12:30 SGT is 10:00 in Kolkata).
 ...
 * truth (see module comment). Accepts a bare calling code ("65") or a full
 * number ("+65 9123 4567"): only the leading digits matter. */
export function guessTimezoneFromCountryCode(countryCodeOrNumber: string): string {
  const digits = countryCodeOrNumber.replace(/\D/g, "");
