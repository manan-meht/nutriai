...
import { isRecommendationSafe } from "./food-balance-safety";

// Mirrors the main web app's src/lib/food-balance/personalize.ts —
...
// manually if the web app's personalization logic changes.

/** Personalizes Food Balance Recommendations using the Food Profile —
 * turns "Add one protein source" into
...
interface PersonalizeOptions {
  goal?: string;
  /** See the main web app's src/lib/food-balance/personalize.ts — same
   * "state the real numbers when available" behavior. */
  macroTargets?: {
...
function buildPersonalizedRecommendationFields(
  ...
  switch (mapping.category) {
    case "protein": {
      ...
      // different words — action is still populated (some callers/tests
      // expect it to be defined) but is no longer rendered as its own line.
