...
const CalorieChart = dynamic(() => import("./MacroCharts").then((m) => m.CalorieChart), { ssr: false });
const CarbsFatChart = dynamic(() => import("./MacroCharts").then((m) => m.CarbsFatChart), { ssr: false });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatCard label="Meals this week" value={String(meals7d.length)} sub={`of ${activeGoal?.targetMealsPerDay ? activeGoal.targetMealsPerDay * 7 : "—"} target`} />
          <StatCard
          ...
          />
          <StatCard label="Avg calories/day" value={avgCalories > 0 ? String(avgCalories) : "—"} sub="kcal" />
          <StatCard label="Active days" value={`${daysActive7d}/7`} sub={`${workouts7d.length} workout${workouts7d.length !== 1 ? "s" : ""}`} />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <ProteinChart meals={meals} targetProteinG={activeGoal?.targetProteinG} />
          <CalorieChart
            meals={meals}
            targetCaloriesMin={activeGoal?.targetCaloriesMin}
            targetCaloriesMax={activeGoal?.targetCaloriesMax}
          />
        </div>
        <CarbsFatChart meals={meals} />
