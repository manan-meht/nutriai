...
interface AddUserImage {
  /** null until a real screenshot/image has been dropped in — renders a
   * placeholder box instead of a broken <Image> tag. */
  src: string | null;
  alt: string;
  caption: string;
  /** Portrait (WhatsApp phone screenshots) vs landscape (dashboard
   * screenshots) — different natural image shapes need different frames. */
...
export const ADD_USER_CONTENT: Record<AddUserVariant, AddUserContent> = {
  family: {
    ...
    image: {
      src: "/landing/adults/add-users/invite-preview.png",
  ...
  coach: {
    ...
    image: {
      src: null, // TODO: public/landing/gym/add-users/dashboard-preview.png
  ...
  self: {
    ...
    image: {
      src: "/landing/self/add-users/whatsapp-preview.png",
...
// Full explainer content — lives on its own dedicated page per flow
// (/family/add-users, /coach/add-users, /me/add-users) rather than inline
// on the marketing pages, which only carry a short teaser link
// (see AddUserTeaser.tsx) pointing here.
export function AddUserExplainer({ variant, ctaHref, ctaLabel }: AddUserExplainerProps) {
  const content = ADD_USER_CONTENT[variant];
    ...
    <section className="py-16 px-6 bg-white">
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4">{content.headline}</h1>
        ...
        {content.image.src ? (
          <div
            className={`mx-auto rounded-3xl overflow-hidden shadow-xl border border-gray-100 ${
              content.image.orientation === "portrait" ? "max-w-[280px]" : "w-full"
            }`}
          >
            <Image
              src={content.image.src}
              alt={content.image.alt}
              width={content.image.orientation === "portrait" ? 560 : 900}
              height={content.image.orientation === "portrait" ? 756 : 675}
          ...
          <div className="rounded-3xl border border-dashed border-gray-200 bg-gray-50 aspect-[4/3] flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-sm font-medium text-gray-500">{content.image.caption}</p>
